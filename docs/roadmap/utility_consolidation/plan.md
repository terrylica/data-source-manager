...
To consolidate and refactor utility files in the `utils` directory to improve maintainability, reduce code duplication, and enhance consistency, without introducing breaking changes to the `core` scripts.
...
- **Tasks**:
  ...
  2. **Create `time_utils.py`**: Create a new file `src/ckvd/utils/time_utils.py`.
  3. **Move Time Functions**: Move relevant functions from `time_alignment.py` and `api_boundary_validator.py` to `src/ckvd/utils/time_utils.py`. Ensure to:
  ...
  4. **Update Imports**: Update all files in `utils` and `core` that currently import from `time_alignment.py` and `api_boundary_validator.py` (for time functions) to import from `src/ckvd/utils/time_utils.py` instead.
  5. **Deprecate Old Functions**: In `time_alignment.py` and `api_boundary_validator.py`, mark the moved time-related functions as deprecated, but keep them as wrappers that call the new functions in `src/ckvd/utils/time_utils.py`. This will provide a transition period and avoid immediate breaking changes. Add clear deprecation warnings using the `warnings` module.
  6. **Testing**:
     ...
     - Create new unit tests specifically for `src/ckvd/utils/time_utils.py` to test all consolidated time functions thoroughly.
...
- **Tasks**:
  ...
  2. **Create `validation_utils.py`**: Create a new file `src/ckvd/utils/validation_utils.py`.
  3. **Move Validation Functions**: Move relevant functions from `cache_validator.py` and `validation.py` to `src/ckvd/utils/validation_utils.py`. Ensure to:
  ...
  4. **Update Imports**: Update all files in `utils` and `core` that currently import from `cache_validator.py` and `validation.py` to import from `src/ckvd/utils/validation_utils.py` for validation functions.
  5. **Deprecate Old Validation Classes/Functions**: In `cache_validator.py` and `validation.py`, mark the moved validation functions and classes as deprecated, using wrapper functions that call the new implementations in `src/ckvd/utils/validation_utils.py`. Add deprecation warnings.
  6. **Testing**:
     - Run existing tests using `scripts/op/run_tests_parallel.sh tests/core tests/utils`.
     - Create new unit tests for `src/ckvd/utils/validation_utils.py` to cover all consolidated validation logic.
...
- **Tasks**:
  ...
  2. **Create `network_utils.py`**: Create a new file `src/ckvd/utils/network_utils.py`.
  3. **Move HTTP Client and Download Functions**: Move relevant classes and functions from `download_handler.py` and `http_client_factory.py` to `src/ckvd/utils/network_utils.py`. Ensure to:
  ...
  4. **Update Imports**: Update imports in `utils` and `core` to use `src/ckvd/utils/network_utils.py` for HTTP client and download functionalities.
  5. **Deprecate Old Classes/Functions**: In `download_handler.py` and `http_client_factory.py`, deprecate the moved classes and functions, providing wrappers that call the new implementations in `src/ckvd/utils/network_utils.py`. Add deprecation warnings.
  6. **Testing**:
     - Run existing tests using `scripts/op/run_tests_parallel.sh tests/core tests/utils`.
     - Create unit tests for `src/ckvd/utils/network_utils.py`, focusing on HTTP client creation, request handling, and download functionalities.
