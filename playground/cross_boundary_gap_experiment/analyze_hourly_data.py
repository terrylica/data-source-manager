#!/usr/bin/env python3
"""
Binance Cross-Day Boundary Gap Analyzer for April 10-11, 2025 Data (1h interval)

This script analyzes Binance 1-hour data for potential gaps across day boundaries,
specifically for April 10-11, 2025.
"""
...
INTERVAL_SECONDS = INTERVAL.to_seconds()  # 3600 seconds for 1 hour
...
def main():
    ...
    logger.info(
        f"Starting analysis of April 10-11, 2025 data with {INTERVAL.value} interval"
    )
    ...
    if "error" in full_days_results:
        ...
        if april_10_data["total_gaps"] > 0:
            ...
            # Print details of all gaps
            logger.info("\nApril 10 Gaps:")
            for i, gap in enumerate(april_10_data["gaps"]):
                logger.info(
                    f"  Gap {i+1}: {gap['from']} to {gap['to']} ({gap['duration_seconds']} seconds)"
                )
        ...
        if april_11_data["total_gaps"] > 0:
            ...
            # Print details of all gaps
            logger.info("\nApril 11 Gaps:")
            for i, gap in enumerate(april_11_data["gaps"]):
                logger.info(
                    f"  Gap {i+1}: {gap['from']} to {gap['to']} ({gap['duration_seconds']} seconds)"
                )
                logger.info(f"    Missing points: {gap['missing_points']}")
