...
- **`okx_symbol_test.py`**:
  ...
  - **Purpose:** Focuses on testing the internal utilities related to OKX symbol handling and endpoint resolution, particularly functions within `utils/market_constraints.py`.
  - **Key Tests:**
...
- **`utils/market_constraints.py`:** Provides enums and functions for market types, data providers, chart types, intervals, symbol formatting, and validation, ensuring tests use consistent and correct parameters.
- **`utils/logger_setup.py`:** Used for logging output within the tests, often utilizing `rich` for formatted console output.
